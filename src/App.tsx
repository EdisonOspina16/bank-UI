...
const PLANS = [
  ...
  },
  {
    ...
    price: '$3,99',
    per: '/mes',
    features: ['Tarjeta física', 'Cambio ilimitado', 'Soporte prioritario', '1% cashback'],
    cta: 'Obtener Plus',
    highlight: true,
  },
  {
    ...
    price: '$13,99',
    per: '/mes',
    features: ['Tarjeta de metal', 'Cashback premium', 'Seguro de viaje', 'Acceso VIP aeropuertos'],
    cta: 'Obtener Metal',
    highlight: false,
  },
]
...
function ExchangeSection() {
  return (
    <section className="py-24 px-6 bg-black text-white">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-zinc-500 mb-6">Divisas</p>
          <h2 className="text-4xl lg:text-5xl font-black leading-tight mb-6">
            La tasa real.<br />Sin sorpresas.
          </h2>
          <p className="text-zinc-400 text-lg leading-relaxed mb-10">
            Cambia entre más de 36 divisas usando el tipo de cambio interbancario real. Lo que ves en pantalla es exactamente lo que recibes.
          </p>
          <a href="#" className="inline-flex items-center gap-2 text-sm font-semibold text-white border-b border-white/30 pb-0.5 hover:border-white transition-colors">
            Ver tipos de cambio en vivo →
          </a>
        </div>

        {/* Exchange widget */}
        <div className="rounded-2xl border border-white/10 p-8" style={{ background: 'rgba(255,255,255,0.04)' }}>
          <p className="text-sm font-semibold text-zinc-400 mb-6">Calculadora de cambio</p>

          <div className="space-y-3 mb-6">
            <div className="rounded-xl bg-white/5 border border-white/8 p-4">
              <p className="text-xs text-zinc-500 mb-2">Tú envías</p>
            ...
            <div className="rounded-xl bg-white/5 border border-white/8 p-4">
              <p className="text-xs text-zinc-500 mb-2">Ellos reciben</p>
              <div className="flex items-center justify-between">
                <span className="text-3xl font-black text-white">4.100.000</span>
                <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg">
                  <span className="text-sm">🇨🇴</span>
                  <span className="text-sm font-semibold">COP</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2.5 mb-6 text-sm">
            {[
              ['Tasa de cambio', '1 USD = 4.100 COP'],
              ['Comisión', 'Gratis'],
              ['Tiempo de llegada', 'Instantáneo'],
            ].map(([k, v]) => (
          ...
          <button className="w-full py-3.5 rounded-xl bg-white text-black text-sm font-bold hover:bg-zinc-100 transition-colors">
            Empezar ahora
          </button>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-px bg-black/8 rounded-2xl overflow-hidden border border-black/8">
          {PLANS.map(plan => (
            <div key={plan.name}
              ...
              <div className="mb-8">
                <p className={`text-xs font-semibold uppercase tracking-widest mb-4 ${plan.highlight ? 'text-zinc-400' : 'text-zinc-400'}`}>
                  {plan.name}
                </p>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-black">{plan.price}</span>
                  {plan.per && <span className={`text-sm ${plan.highlight ? 'text-zinc-400' : 'text-zinc-400'}`}>{plan.per}</span>}
                </div>
              </div>
